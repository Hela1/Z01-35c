...
def chatUnreadMessages(id):
    i = 0
    for m in Message.query.filter_by(id=id):
        if m.wasRead == False and m.idAuthor!=current_user:
            i+=1
    return i

def chatLastMessages(id):
    i = Message.query.filter_by(id=id)
    if i:
        result = None
        for m in i:
            result = m.date
...
def chatPalnel():
    ...
    for ch in Chat.query.all():
        if ch.user1 == current_user.username:
            ...
            unreadMessages = 'Brak nowych wiadomości'
            if chatUnreadMessages(ch.id) != 0:
                unreadMessages = f" ({chatUnreadMessages(ch.id)}) "
    ...
    if chat_form.validate_on_submit():
        ...
        if user_obj:
            if findUserChat(current_user.username, user):
                ...
                if Chat.query.filter_by(user1=current_user.username, user2=user).first() or Chat.query.filter_by(user1=user, user2=current_user.username).first():
                    return "1,5 Arbuza"
                else: 
...
def userchat(id_chatu):
    ...
    for msg in msg_l:
        msg_list.append((msg.idAuthor, msg.text, msg.wasRead, msg.date))
        if not msg.wasRead and msg.idAuthor != current_user.username:
            print('lol')
            msg.wasRead = True
            print(Message.query.filter_by(id=msg.id).first().wasRead)
            db.session.commit()
            print(Message.query.filter_by(id=msg.id).first().wasRead)
    ...
    if chat.user1 == current_user.username:
        return render_template("chats.html", username=current_user.username, user2=chat.user2, msg_form=msg_form, msg=msg_list)
    return render_template("chats.html", username=current_user.username, user2=chat.user1, msg_form=msg_form, msg=msg_list)
